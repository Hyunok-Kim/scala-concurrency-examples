...
object SchedulersBrowser extends scala.swing.SimpleSwingApplication {
  ...
  trait BrowserLogic {
    ...
    def suggestRequest(term: String): Observable[String] = {
      ...
      Observable.from(request).timeout(0.5.seconds).onErrorResumeNext(t => Observable.just("(no suggestions)"))
    }

    def pageRequest(url: String): Observable[String] = {
      val request = Future { Source.fromURL(url).mkString }
      Observable.from(request).timeout(4.seconds).onErrorResumeNext(t => Observable.just(s"Could not load page: $t"))
    }
