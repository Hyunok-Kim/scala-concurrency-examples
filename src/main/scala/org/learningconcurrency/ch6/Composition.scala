...
object CompositionConcatAndFlatten extends App {
  ...
  def fetchQuote(): Future[String] = Future {
    blocking {
      ...
      val parsed = ujson.read(Source.fromURL(url).getLines().mkString)
      val arr = parsed.arr.map(_("text").str)
      arr(Random.between(0, arr.size))
    }
  }
...
object CompositionRetry extends App {
  ...
  def randomQuote = Observable[String] { sub =>
    val url = "https://type.fit/api/quotes"
    val parsed = ujson.read(Source.fromURL(url).getLines().mkString)
    val arr = parsed.arr.map(_("text").str)
    sub.onNext(arr(Random.between(0, arr.size)))
    sub.onCompleted()
  }
  ...
  def shortQuote = for {
    txt <- randomQuote
    message <- if (txt.length < 60) just(txt) else errorMessage
  } yield message
...
object CompositionErros extends App {
  import rx.lang.scala._
