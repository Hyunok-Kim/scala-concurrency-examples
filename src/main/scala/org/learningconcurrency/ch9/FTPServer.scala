...
class FileSystem(val rootpath: String) {
  ...
  def init() = atomic { implicit txn =>
    ...
    for (file <- fileIterator if file.getName != "") {
      val info = FileInfo(file)
      files(info.path) = if (info.parent != null) info.copy(parent = "." + File.separator + info.parent) else info.copy(parent = ".")
    }
  }

  def getFileList(dir: String): Map[String, FileInfo] = atomic { implicit txn =>
    val files2 = files.filter(_._2.parent == dir)
    files2
  }
  ...
  def findFiles(regex: String): Seq[FileInfo] = {
    ...
    /* infos.par => ParArray, info.par.filter(...).seq => ArraySeq */
  }
}
